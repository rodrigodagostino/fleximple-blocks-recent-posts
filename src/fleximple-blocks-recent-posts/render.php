...
$rel_attribute = trim(($attributes['noFollow'] ? 'nofollow' : null) . ' ' . ($attributes['noReferrer'] ? 'noreferrer' : null));
	...
	// if (isset($attributes['selectedPostsIds'])) {
	// 	$recent_posts = array();
	// 	foreach ($attributes['selectedPostsIds'] as $post) {
...
foreach ($recent_posts as $post) {
	...
	if (isset($attributes['displayCategories']) && $attributes['displayCategories']) {
		...
		foreach ($categories as $category) {
			$categories_links .= '<a class="' . $default_class_name . '__entry-category" href="' . esc_url(get_category_link($category->term_id)) . '" rel="category" data-category-slug="' . esc_html($category->slug) . '"><span class="screen-reader-only">' . __('Category:', 'fleximple-blocks-recent-posts') . '</span>' . esc_html($category->name) . '</a>';
		}
	...
	if (isset($attributes['displayTitle']) && $attributes['displayTitle']) {
		...
		if (!$post_title) {
			$post_title = __('(no title)', 'fleximple-blocks-recent-posts');
		}
	...
	if (isset($attributes['displayAuthor']) && $attributes['displayAuthor']) {
		$post_author = sprintf(
			...
			$default_class_name,
			__('Published by:', 'fleximple-blocks-recent-posts'),
	...
	if (isset($attributes['displayDate']) && $attributes['displayDate']) {
		$post_date = sprintf(
			...
			$default_class_name,
			__('Published on:', 'fleximple-blocks-recent-posts'),
	...
	if (!empty($attributes['displayComments'])) {
		...
		$comments_number = sprintf(_n('%d comment', '%d comments', $comments_number, 'fleximple-blocks-recent-posts'), $comments_number);
