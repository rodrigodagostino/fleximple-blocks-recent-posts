...
export default function RecentPostsSelectControl({
	...
	return (
		<>
			<BaseControl
				label={__('Posts', 'fleximple-blocks-recent-posts')}
			...
			>
				<AsyncSelect
					...
					placeholder={__('Search a post…', 'fleximple-blocks-recent-posts')}
					defaultValue={selectedPosts}
					onChange={(selectedOptions) => setAttributes({ selectedPosts: selectedOptions })}
					isMulti={true}
					loadingMessage={() => __('Loading…', 'fleximple-blocks-recent-posts')}
					noOptionsMessage={() => __('No posts found.', 'fleximple-blocks-recent-posts')}
