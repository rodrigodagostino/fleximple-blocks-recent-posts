				...
				return (
					<article key={i} id={`post-${post.id}`} className={`${defaultClassName}__entry`}>
									...
									<div key={j} className={`${defaultClassName}__entry-media`}>
										{orderMedia.map((mediaFragment, k) => {
											if (mediaFragment === 'featuredImage' && displayFeaturedImage) {
																			...
																			key !== 'small'
																				? `(min-width: ${
																						fleximpleblocksPluginData.settings[
																							array[x + 1][0] + 'BreakpointValue'
																						]
																					}px)`
																				: null
																		}
																		srcSet={featMediaItems[i]?.[key]?.source_url}
																	/>
																);
															}
														});
												...
												return (
													<picture key={k} className={`${defaultClassName}__entry-picture`}>
							...
							if ('content' === fragment && displayContent) {
									...
									<div key={j} className={`${defaultClassName}__entry-content`}>
										{orderContent.map((contentFragment, k) => {
											if (
												...
												return (
													<div key={k} className={`${defaultClassName}__entry-categories`}>
											...
											if (contentFragment === 'title' && displayTitle && !!post.title.rendered) {
												return (
													<TagName key={k} className={`${defaultClassName}__entry-title`}>
														{/* <a
                              href={ post.link }
                              target="_blank"
                              rel={ relAttribute }
                              data-link-name="article"> */}
											...
											if (
												...
												return (
													<div key={k} className={`${defaultClassName}__entry-meta`}>
														{orderMeta.map((metaFragment, l) => {
															...
															if (metaFragment === 'comments') {
																if (displayComments && !!post.comments_number) {
																	return (
																		<span key={l} className={`${defaultClassName}__entry-comments`}>
											...
											if (contentFragment === 'excerpt' && displayExcerpt) {
												return (
													<RawHTML key="html" className={`${defaultClassName}__entry-excerpt`}>
						...
						<a
							...
							dangerouslySetInnerHTML={{
								__html: `${__('Edit', 'fleximple-blocks-recent-posts')} «${post.title.rendered}»`,
							}}
						/>
					</article>
				);
